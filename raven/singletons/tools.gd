...
const OPTIONS_WINDOW: PackedScene = preload("res://raven/game/ui/options/options_window.tscn")

func get_options_window(close_func: Callable = func(): pass):
...
func lerp_fix(x: float, y: float, delta: float, speed: float) -> float:
	return lerpf(y, x, exp(-delta * speed))

func switch_scene(scene: PackedScene, _skip_transition: bool = false):
...
func refresh_scene(_skip_transition: bool = false):
	if Settings.skip_transitions: _skip_transition = true
	if not _skip_transition: await Transition.transition_in()
	get_tree().reload_current_scene()
	if not _skip_transition: await Transition.transition_out()

func deffered_scene_call(fn: String, args: Array = []):
	deffered_call(get_tree().current_scene, fn, args)

func deffered_call(node: Node, fn: String, args: Array = []):
	if node == null: return
	if node.has_method(fn): node.callv(fn, args)

func begin_flicker(node: CanvasItem, duration: float = 1.0, interval: float = 0.04,
	visible_when_finished: bool = false, finished_callback: Callable = func(): pass):
	####
	if node == null: return
