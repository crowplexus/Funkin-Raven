...
func reload_intro_text() -> void:
	var intro_file: FileAccess = FileAccess.open("res://assets/data/introText.csv", FileAccess.READ)
	var err: = FileAccess.get_open_error()

	if intro_file == null:
		print_debug("intro text file coul	d not be loaded, Error ", err)
		return
	...
	if split_text != null and not split_text.is_empty():
		intro_blobs.clear()
...
func create_text_blob(text: PackedStringArray, spacing: int = 60) -> void:
...
func add_text_blob(text: String, spacing: int = 60) -> void:
