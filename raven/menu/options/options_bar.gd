...
@export_multiline var description: String = 'Very simple really...' ## The description that gets shown in the box below!!
...
func update_setting(amnt: int = 0, precache: bool = false) -> void:
	...
	match variable_type: # set name
		...
		3: # Array
			if typeof(val) == TYPE_INT: val_name = str(choice_names[val % choice_names.size()])
...
func reload_name() -> void:
	if name_label != null:
		var translated: String = tr("setting_%s" % name)
		name_label.text = setting if translated.begins_with("setting_") else translated

func reload_value_name() -> void:
	var _temp_val_name: String = val_name
	var translated: String = tr("value_" + val_name.to_snake_case())
	if not translated.begins_with("value_"):
		_temp_val_name = translated

	if option_status != null:
		option_status.text = value_format.replace('@', _temp_val_name)
